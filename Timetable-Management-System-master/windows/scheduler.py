...
def sendgmail():
    ...
    fromaddr = "fortunemvpafrom@gmail.com"
    frompassword="hzngtflxyhxldjpw"
   
    body = "Please note your timetable, it has been changed, please go through the attachment"
    subject="Timetable Change Notification"

    batch=str(combo1.get())
    # cursor=conn.execute("select email from student where section='{batch}'")
    cursor=conn.execute("select email from student where section='" + batch + "'")
    cursor = list(cursor)
    cnt=len(cursor)
    to=[]
    for i in range(0,cnt):
        to.append(cursor[i][0])
        print(cursor[i][0])
        toaddr=cursor[i][0]
        ff = randrange(10000,99999)
        filename = os.getcwd() + "/screen/screen_" +  str(ff) + ".jpg"
        ...
        #filename = "C:\\DATA\\Projects\\2023\\Viswajyothi\\Timetable-Management-System-master\\screen\\screen_91058.jpg"
...
title_lab = tk.Label(
    ...
    font=('areal', 20, 'bold'),justify='center',
    pady=5
)
title_lab.pack()
